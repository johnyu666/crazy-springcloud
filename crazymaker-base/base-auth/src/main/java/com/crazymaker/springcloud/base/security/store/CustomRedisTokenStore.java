package com.crazymaker.springcloud.base.security.store;
...
public class CustomRedisTokenStore implements TokenStore
{
    ...
    private int getAccessTokenValiditySeconds(String clientId)
    {
        ...
        try
        {
            bytes = conn.get(serializeKey(SessionConstants.CACHE_CLIENT_KEY + ":" + clientId));
        } finally
        {
            conn.close();
        }
        ...
        //返回默认值
        return SessionConstants.ACCESS_TOKEN_VALIDITY_SECONDS;
    }
    ...
    public OAuth2Authentication readAuthentication(String token)
    {
        ...
        try
        {
            bytes = conn.get(serializeKey(SessionConstants.REDIS_TOKEN_AUTH + token));
        } finally
        {
            conn.close();
        }
    ...
    private void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication, boolean isRenew)
    {
        ...
        byte[] authKey = serializeKey(SessionConstants.REDIS_TOKEN_AUTH + token.getValue());
        byte[] authToAccessKey = serializeKey(AUTH_TO_ACCESS + authenticationKeyGenerator.extractKey(authentication));
        byte[] approvalKey = serializeKey(SessionConstants.REDIS_UNAME_TO_ACCESS + getApprovalKey(authentication));
        byte[] clientId = serializeKey(SessionConstants.REDIS_CLIENT_ID_TO_ACCESS + authentication.getOAuth2Request().getClientId());
    ...
    public void removeAccessToken(String tokenValue)
    {
        ...
        byte[] authKey = serializeKey(SessionConstants.REDIS_TOKEN_AUTH + tokenValue);
        ...
        try
        {
            ...
            if (authentication != null)
            {
                ...
                byte[] unameKey = serializeKey(SessionConstants.REDIS_UNAME_TO_ACCESS + getApprovalKey(authentication));
                byte[] clientId = serializeKey(SessionConstants.REDIS_CLIENT_ID_TO_ACCESS + authentication.getOAuth2Request().getClientId());
                conn.openPipeline();
    ...
    {
        byte[] approvalKey = serializeKey(SessionConstants.REDIS_UNAME_TO_ACCESS + getApprovalKey(clientId, userName));
        List<byte[]> byteList;
        RedisConnection conn = getConnection();
        try
        {
    ...
    {
        byte[] key = serializeKey(SessionConstants.REDIS_CLIENT_ID_TO_ACCESS + clientId);
        List<byte[]> byteList;
        RedisConnection conn = getConnection();
        try
        {
