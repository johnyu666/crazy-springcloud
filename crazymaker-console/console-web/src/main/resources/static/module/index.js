...
layui.define(['config', 'admin', 'layer', 'laytpl', 'element', 'form'], function (exports) {
    ...
    var index = {
        ...
        initLeftNav: function () {
            admin.req('console-provider/menus/current', {}, function (data) {
                //data = data[1];
        ...
        getUser: function (success) {
            layer.load(2);
            admin.req('console-provider/users/current', {}, function (data) {
                // alert(data);
                layer.closeAll('loading');
                if (data && data.respCode === 0) {
                    // alert("获取用户成功");
                    let user = data.datas;
                    config.putUser(user);
        ...
        bindEvent: function () {
            ...
            $('#btnLogout').click(function () {
                layer.confirm('确定退出登录？', function () {
                    //通过认证中心 tuic
                    admin.req('console-provider/oauth/remove/token', {}, function (data) {
                        config.removeToken();
                        location.replace('login.html');
                    }, 'POST');
                });
